...
void v1190a::TestWR(Data16 value){ //Test : try to write/read 16 bit word
    for(int tdc=0; tdc < MAXNTDC; tdc++){
        ...
        if(test == value) MSG_INFO("[v1190a]: TDC%i result of W/R test is : %X (%X tested)\n",tdc,test,value);
        else MSG_ERROR("[v1190a]: TDC%i result of W/R test is : %X (%X tested)\n",tdc,test,value);
    }
}
...
void v1190a::CheckStatus(){ //Status of the TDCs (Status register)
    ...
    for(int tdc=0; tdc < MAXNTDC; tdc++){
        ...
        MSG_INFO("[v1190a]: TDC%i STATUS is : %X\n",tdc,status);
    }
}
...
void v1190a::GetTrigConfiguration(){ //Read and print trigger configuration
    MSG_INFO("[v1190a]: Control of the trigger configuration:\n");
...
void v1190a::SetTDCDetectionMode(Data16 mode){
    MSG_INFO("[v1190a]: Set edge detection\n");
...
void v1190a::SetTDCResolution(Data16 lsb){ //Resolution readout
    MSG_INFO("[v1190a]: Set channel resolution\n");
...
void v1190a::SetTDCDeadTime(Data16 time){
    MSG_INFO("[v1190a]: Set channel dead time between hits\n");
...
void v1190a::Set(IniFile * inifile){
    MSG_INFO("[v1190a]: START TO SET THE TDC\n");
